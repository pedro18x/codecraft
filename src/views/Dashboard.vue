...
const startDailyChallenge = () => {
  ...
  startProblem(dailyChallenge.value.id)
}
...
<template>
  ...
  >
    <template #topbar-actions>
      <BrutalButton variant="secondary" size="sm" :disabled="!dailyChallenge" @click="startDailyChallenge">
      ...
      >
        ...
        <BrutalCard variant="flat" padding="lg">
          <div class="rail-section">
            ...
            <div class="rail-checks" role="radiogroup" aria-label="Filter by status">
              <BrutalCheckbox
                v-for="opt in (['all', 'todo', 'attempted', 'solved'] as const)"
                :key="opt"
                type="radio"
                :model-value="statusFilter"
                :value="opt"
                :label="opt.charAt(0).toUpperCase() + opt.slice(1)"
                @update:model-value="statusFilter = opt"
              />
            </div>
          </div>
